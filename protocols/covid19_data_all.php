...
if ($type == "cell") {
    ...
    if ($requestData['search']['value']) {
        $query2 = "select distinct 
			...
			patient_sample , patient_sample_date,patient_evaluation_date from micha_protocols p  where protocol_name='$protocol' 
    ...
    if ($requestData['search']['value']) {
        $query2 = "select distinct    round(CAST( min_concentration as numeric)   ,4) as min_concentration,   
			   round(CAST( max_concentration as numeric) ,4) as max_concentration,
						analysis_ref, analysis_metric from micha_protocols p  where protocol_name='$protocol' 
			  ...
			  or analysis_metric LIKE '%" . strtoupper($requestData['search']['value']) . "%' )			  
						 ";
        //REPLACE(min_concentration, ',', '.')
        $query3 = "select distinct   round(CAST( min_concentration as numeric)   ,4) as min_concentration,   
			   round(CAST( max_concentration as numeric) ,4) as max_concentration,
						analysis_ref, analysis_metric from micha_protocols p  where protocol_name='$protocol' 
						 and (min_concentration LIKE '%" . strtoupper($requestData['search']['value']) . "%' 
			  or max_concentration LIKE '%" . strtoupper($requestData['search']['value']) . "%'
        ...
        $query2 = "select distinct  round(CAST( min_concentration as numeric)   ,4) as min_concentration,   
			   round(CAST( max_concentration as numeric) ,4) as max_concentration,
						analysis_ref, analysis_metric from micha_protocols p  where protocol_name='$protocol' 
						 ";
        $query3 = "select distinct   round(CAST( min_concentration as numeric)   ,4),   
			   round(CAST( max_concentration as numeric) ,4),analysis_ref, 
			   analysis_metric from micha_protocols p  where protocol_name='$protocol' ";
    }
    ...
    if ($requestData['search']['value']) {
        ...
        $query2 = "	select distinct dilution_fold , vehicle , experimental_medium , plate_type , 
				assay_format , assay_test_type , detection_tech  , cell_density , method_dispensation , 
			    volume_per_well , treatment_time from micha_protocols p  where protocol_name='$protocol' 
		...
		$query3 = "	select distinct dilution_fold , vehicle , experimental_medium , plate_type , 
				assay_format , assay_test_type , detection_tech  , cell_density , method_dispensation , 
				volume_per_well , treatment_time from micha_protocols p  where protocol_name='$protocol' 
				and (dilution_fold LIKE '%" . strtoupper($requestData['search']['value']) . "%' 
			    or vehicle LIKE '%" . strtoupper($requestData['search']['value']) . "%'
    ...
    if ($requestData['search']['value']) {
        $query2 = "select distinct compound_name , standard_inchi_key from micha_protocols p  where protocol_name='$protocol' 
				and (compound_name LIKE '%" . strtoupper($requestData['search']['value']) . "%' 
			    or standard_inchi_key LIKE '%" . strtoupper($requestData['search']['value']) . "%') 
			                ";
        $query3 = "select distinct 
						   compound_name , standard_inchi_key from micha_protocols p  where protocol_name='$protocol' 
			  and (compound_name LIKE '%" . strtoupper($requestData['search']['value']) . "%' 
			  or standard_inchi_key LIKE '%" . strtoupper($requestData['search']['value']) . "%')  ";
    } else {
        $query2 = "select distinct 
