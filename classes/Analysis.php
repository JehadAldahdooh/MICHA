...
class Analysis {	
	...
	public function updateRecord(){
		...
		$stmt = $this->conn->prepare("
			...
			SET  citation = ?, analysis_result = ?,  min_concentration = ?, max_concentration = ? 
			WHERE id = ?");
		  ...
		  // 	$this->analysis_pipeline_address = htmlspecialchars(strip_tags($this->analysis_pipeline_address));
			...
			$stmt->bind_param("ssssi",  $this->citation,
	...
	public function addRecord(){
		...
		//if($this->analysis_normalization) {
			...
			$stmt = $this->conn->prepare("
			INSERT INTO ".$this->recordsTable."(`session_id`,`file_name`,
			`citation`,`analysis_result`,`min_concentration` ,`max_concentration`) 
			 VALUES(?,?,?,?,?,?)");
...
echo $this->conn->error;
			$stmt->bind_param("ssssss", $this->sid,$this->filename, 
                             			$this->citation,
                            			$this->analysis_result, 
